...
public:
    ...
    bool Contains(const T& _key,int hash=-1);
private:
    H HashFunction;
...
bool HashTable<T,H>::Add(const T& _key) {
    ...
    int cnt = 1;unsigned int probe = hash;
    while (table[probe].type != 0 && table[probe].key  != _key && cnt < table.size()) {
        if(table[hash].type != -1)
            hash = (hash + cnt + 1)%table.size();
        probe = (probe + cnt + 1)%table.size();
        cnt++;
    }
    if(cnt == table.size() || table[probe].key  == _key)
        return false;
    table[hash].key = _key;
    table[hash].type = 1;
    KeysCount++;
...
bool HashTable<T,H>::Contains(const T& _key,int hash){
    if(hash == -1)
        hash = HashFunction(_key) % table.size();
    int i = 1;
